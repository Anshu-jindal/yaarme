...
function slideshow(section) {
  ...
  if (index === 1) {
    setTimeout(() => {
      bars[0].querySelector("div").classList.add("load-now");
    }, 2);
  }

  for (var i = 0; i <= index - 2; i++) {
    setTimeout(() => {
      bars[i].querySelector("div").classList.remove("load-now");
    }, 2);
    bars[i].querySelector("div").style.width = "100%";
  }

  setTimeout(() => {
    bars[index - 1].querySelector("div").classList.add("load-now");
  }, 2);
...
function leftTouch() {
  ...
  if (index >= 1) slideshow(sections[nxt - 1]);
  else {
    prevClick();
  }
}
